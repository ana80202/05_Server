...
public class PizzaOrderController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		pizzaList.add( new Pizza("하와이안 골든링 피자",10000) );
		pizzaList.add( new Pizza("스테이크 치즈 피자",14000) );
		...
		//피자 종류 파악
		//-> pizza는 "1" 처럼 문자열로 넘어온다. -> 정수로 형변환(parsing)이 필요하다.
		Pizza p = pizzaList.get( Integer.parseInt(pizza) - 1 );
		// 0번재 인덱스 치즈피자 1번쨰 인덱스 하와이안 골든링 2번째 스테이크치즈
