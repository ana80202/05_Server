...
public class ScopeController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//프로그램상 사용되는 변수들은 사용 가능한 범위를 가지는데, 
		//이 때 변수의 범위를 Scope 스코프라고 한다.
		...
		//4.appliection scope -> 서버가 켜져있는 동안 유지
		
		//application 객체 얻어오는 방법
		//-> request,session 객체에서 얻어오기 가능
		 ServletContext application = req.getServletContext();
		 //ServletContext application = ㅊ;
		 //  req.getServletContext(); /  req.getServletContext(); 둘다 가능하다.
